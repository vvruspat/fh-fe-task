...
export const SearchForm = () => {
    ...
    const onSelectGuestsClick = useCallback((event) => {
        event.preventDefault();
    ...
    return <form onSubmit={onSubmitSearch} className="search-form">
        <Input before={<Icon name="pointer" />} after={<Icon name="geo" />} placeholder="Type city, place, or hotel name" onChange={onLocationChange} />
        <div className="date-and-guests__row">
            <DateRangePicker className="search-form-date" onRangeSelect={onRangeSelect} />
            <Button className="search-form-guests" mode="outline" before={<Icon name="guests" />} onClick={onSelectGuestsClick}>{guestsNumber}</Button>
        </div>
        <Button stretched type="submit" disabled={isDisabled} onClick={onSearchClick}>Search</Button>
    </form>
}