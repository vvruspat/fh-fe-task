...
import { addRoom, removeRoom, updateRoom } from "../../redux/rooms/roomsSlice";
...
export const Rooms = () => {
  ...
  const onRoomUpdateAdults = useCallback(
    (id: string, adults: number) => {
  ...
  useEffect(() => {
    if (rooms.length > 1) {
      roomsRef.current?.scrollTo?.({
        top: roomsRef.current.scrollHeight + 248,
      });
    }
  }, [rooms.length]);
  ...
  return (
    <div className="rooms">
      <div
        className={cn("rooms-wrapper", {
          extended:
            (roomsRef.current?.scrollHeight ?? 0) >
            (roomsRef.current?.offsetHeight ?? 0),
        })}
        ref={roomsRef}
      >
